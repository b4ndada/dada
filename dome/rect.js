...
;(function(){
    ...
    */
	...
	Adproto.init = function(){
		...
		$element.className = 'dmui rect '+ settings['contSize'];
		//业务
		_body = _this.getBoby();
	...
	Adproto.getBoby = function(){
		var _html = [];
		_html.push('<div class="dmui-content" style="">');
		_html.push(		'<div class="dmui-logo" style=""><a href="{logolink}" target=\"_blank\"><img src="{logo}" alt="logo" /></a></div>');
	...
	Adproto.getItems = function(items){
		...
		for(; i<_len; i++){
			...
			_html.push('<li>');
			_html.push(		'<div class="dmui-img"><a href=\"'+items[i]['linkUrl']+'\" title=\"'+_title+'\" target=\"_blank\"><img src=\"'+items[i]['mediaSrc']+'\" alt=\"'+_title+'\"></a></div>');
